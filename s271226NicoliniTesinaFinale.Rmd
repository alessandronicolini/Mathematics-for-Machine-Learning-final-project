...
knitr::include_graphics(here::here("/home/alessandro/Documents/DataScienceEng/1_2/Mathematics_in_Machine_learning/NOxEmissionData/gas_turbine.png"))
```
...
The data has been already split into train, val and test and EDA is performed only to the train an val part merged together. The data exploratory phase starts with a look at the output variable empirical density in figure \@ref(fig:originalResponseFig). The *histogram* is an *approximated representation* of the real NO~x~ density function, it gives a rough sense of the density associated to the underlying distribution of the NO~x~ data. The range of the values that NO~x~ can take is divided into intervals of equals width. The height of each bar equals the *empirical density* associated to the corresponding interval, and it is defined as the *empirical probability* divided by the interval width, where the probability is computed as the number of sample observations within the interval divided by the total number of observations. It appears to be positively skewed, and this is a problem because the normality of the dependent variable is an assumption in performing linear regression. The Q-Q plot clearly highlights the normality assumption violation.
...
respHist <- ggplot() +
  geom_histogram(data = emissions[-test, ], aes(x = NOX, y = ..density..), fill="#3b3b9c") +
  xlab("NOx")+
  ylab("emp. density")+
...
```{r newResponseFig, fig.align="center", fig.width = 7.5, fig.height = 3, message = FALSE, warning = FALSE, fig.cap = "bcNO~x~ empirical density distribution."}

# bcNOX empirical density ------------------------------------------------------
transHist <- ggplot() +
  geom_histogram(data = emissions[-test, ], aes(x = bcNOX, y = ..density..), fill="#3b3b9c")+
...
Identifying closely related predictor variables is an important step in a regression context, in fact problems could arise since it is difficult to separate out the individual effect on the response of this kind of variables. The easiest solution adopted in this work is to eliminate one of the highly correlated features. Two are the main limits of the Pearson's correlation coefficient:
...
- $\boldsymbol{Y}$ is the $n \cdot 1$ *response vector* 

- $\boldsymbol{X}$ is $n\cdot p$ matrix containing the values of $p-1$ predictors because the first column is usually a column of $1$'s. They are not random but can be considered as values under the control of the experimenter

- $\boldsymbol{\beta}$ is the $p \cdot 1$ vector of unknown parameters object of the statistical inference

- $\boldsymbol{\epsilon}$ is a $n\cdot1$ vector of *unobservable* random variables which take into account the error due to different sources of uncertainty (simplicity of the model, measurement errors, natural variability, ...). 
...
\boldsymbol{\epsilon} \sim \mathcal{N}_n\big(\boldsymbol{0}_{n \cdot 1},\; \sigma^2\boldsymbol{I}_{n \cdot n}\big)
...
\boldsymbol{Y} \sim \mathcal{N}_n\big(\boldsymbol{X\beta},\; \sigma^2\boldsymbol{I}_{n \cdot n}\big)
...
meaning that the effect of $AT$ on $bcNO_x$ is no longer constant. $\beta_3$ can be considered as the increase in the $AT$ contribution for a unit increase in $AH$. Right side of figure \@ref(fig:3dplot) shows how the least squares surface now better fit the data.
...
## Model selection
### Models comparison via Anova and AIC
...
where $\hat{L}$ is the maximized likelihood function and a low AIC value means the associated model is a good one.

```{r}
# AIC ==========================================================================
as.matrix(AIC(simple_lm, multiple_lm, interaction_lm, big_lm, big_interaction_lm))
```

Comparing all the models we defined we see that the `big_interaction_model` is the better one among them, although having the higher number of parameters.
...
### Model performance
To finally select the best model we compute the model performance on the validation set, in the original space, inverting the transformation on the predicted values; only then we can use the predicted values to compute the previously introduced [evaluation metrics](#evaluation-metrics).
...
```
As we can see the `big_interaction_lm` yields the lowest error with respect to the other models. 
